...
fn test_is_corruption_error_detects_lance_not_found() {
    ...
    assert!(is_corruption_error(&err), "should detect lance file not found");
}
...
fn test_is_corruption_error_detects_execution_error_not_found() {
    ...
    assert!(is_corruption_error(&err), "should detect execution error not found");
}
...
fn test_is_corruption_error_ignores_normal_errors() {
    ...
    assert!(!is_corruption_error(&err1), "should not detect connection refused as corruption");
    
    let err2 = anyhow::anyhow!("timeout waiting for response");
    assert!(!is_corruption_error(&err2), "should not detect timeout as corruption");
    
    let err3 = anyhow::anyhow!("permission denied");
    assert!(!is_corruption_error(&err3), "should not detect permission denied as corruption");
    
    let err4 = anyhow::anyhow!("no such file or directory: config.yaml");
    assert!(!is_corruption_error(&err4), "should not detect missing config as corruption");
}
...
fn test_clear_corrupted_index_nonexistent_returns_false() {
    ...
    assert!(result.is_ok());
    assert_eq!(result.unwrap(), false, "should return false for nonexistent path");
}
...
fn test_clear_corrupted_index_creates_backup() {
    ...
    if backup_dir.exists() {
        ...
        println!("Found {} corrupted backups in {:?}", backups.len(), backup_dir);
    }
}
...
async fn status_reports_corruption_when_lance_files_missing() {
    ...
    assert_eq!(result["exists"], false, "nonexistent db should report exists=false");
...
async fn startup_check_returns_none_when_no_index_exists() {
    ...
    assert_eq!(result["action"], "none", "should return action=none when no index exists");
...
async fn startup_check_returns_none_for_invalid_root() {
    ...
    assert_eq!(result["action"], "error", "should return action=error for invalid root");
    assert!(result["message"].as_str().unwrap().contains("Invalid root path"));
...
async fn startup_check_detects_corruption_and_triggers_rebuild() {
    ...
    assert!(result["corrupted"].is_boolean(), "corrupted should be a boolean");
    assert!(result["indexed"].is_boolean(), "indexed should be a boolean");
    assert!(result["watching"].is_boolean(), "watching should be a boolean");
...
fn test_validate_lance_table_nonexistent_returns_false() {
    ...
    assert!(!validate_lance_table(&path), "should return false for nonexistent path");
}
...
fn test_validate_lance_table_empty_dir_is_valid() {
    ...
    assert!(validate_lance_table(&table_path), "empty table dir should be valid");
}
...
fn test_validate_lance_table_with_data_files_is_valid() {
    ...
    // Create a .lance file
    std::fs::write(data_dir.join("abc123.lance"), "fake lance data").unwrap();
    
    assert!(validate_lance_table(&table_path), "table with .lance files should be valid");
}
...
fn test_validate_lance_table_empty_data_dir_is_corrupted() {
    ...
    assert!(!validate_lance_table(&table_path), "empty data dir should be invalid");
}
...
fn test_validate_lance_table_wrong_extension_is_corrupted() {
    ...
    assert!(!validate_lance_table(&table_path), "data dir without .lance files should be invalid");
}
...
fn test_is_database_corrupted_detects_missing_chunks_data() {
    ...
    assert!(errors.iter().any(|e| e.contains("chunks.lance")), "error should mention chunks.lance");
}
...
fn test_is_database_corrupted_detects_missing_config_data() {
    ...
    assert!(errors.iter().any(|e| e.contains("config.lance")), "error should mention config.lance");
}
...
async fn test_storage_open_auto_recovers_from_corruption() {
    ...
    assert!(result.is_ok(), "Storage::open should succeed after auto-recovery: {:?}", result.err());
    
    // Database should no longer be corrupted
    let (corrupted_after, errors) = is_database_corrupted(&db_path);
    assert!(!corrupted_after, "should not be corrupted after open: {:?}", errors);
}
...
async fn test_storage_open_creates_fresh_db() {
    ...
    assert!(result.is_ok(), "Storage::open should create fresh db: {:?}", result.err());
...
async fn test_get_config_returns_none_on_missing_key() {
    ...
    // Set a config value
    storage.set_config("test_key", "test_value").await.expect("set config");
